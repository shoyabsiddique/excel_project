...
#             'units': 'UNITS'
#         }
#       **Important:** Include any additional columns (e.g. 'mah') needed for placeholder substitution.
...
#     # If needed, fill missing values for other key columns.
#     # For example, if you expect a "mah" column, you might want:
#     # df['mah'] = df['mah'].fillna("0")
...
#     # Build a dictionary from rough description to the formatted string.
#     description_mapping = {item['rough']: item['formatted'] for item in name_mapping}
    
#     # Define a function to perform substitution for placeholders.
#     def substitute_placeholders(row, mapping, col_config):
#         # Get the current description.
#         current_desc = row[col_config['description']]
#         # If the description exists as a key in the mapping,
#         # use its formatted string.
#         if current_desc in mapping:
#             formatted_str = mapping[current_desc]
#             # Find all placeholders of the form %placeholder%
#             placeholders = re.findall(r'%([^%]+)%', formatted_str)
#             print(placeholders)
#             for ph in placeholders:
#                 # Only replace if the column exists in the row.
#                 # print(row)
#                 if ph in row:
#                     formatted_str = formatted_str.replace(f'%{ph}%', str(row[ph]))
#             return formatted_str
#         return current_desc
#     df['Block'] = (df[column_config['pcs_per_ctn']] != df[column_config['pcs_per_ctn']].shift()).cumsum()
#     df[column_config['description']] = df.apply(
#         lambda row: substitute_placeholders(row, description_mapping, column_config),
#         axis=1
#     )
...
#     # Generate the CTN NO field dynamically using pd.notnull() for missing values.
...
#     # Apply the substitution function row by row.
#     final_dataset[column_config['description']] = final_dataset.apply(
#         lambda row: substitute_placeholders(row, description_mapping, column_config),
#         axis=1
#     )
    
#     # Optionally, drop rows with a specific description (e.g., those with "Noen" or "Unknown").
#     # Adjust the string as needed. Here we drop rows where the description equals "Noen".
...
# # Ensure that if you want to use a placeholder like %mah%, the column "mah" must be kept.
# column_config = {
#     'mark': 'MARK',
#     'description': 'DESCRIPTION',
#     'pcs_per_ctn': 'PCS/CTN',
...
#     'weight_total': 'WEIGHT/TOTAL',
#     'units': 'UNITS',
#     # Include any additional columns you need for substitution:
#     'mah': 'MAH',
#     'bis no.': 'BIS NO.',
#     'bis model no.': 'BIS MODEL NO.',
# }

# # If you previously dropped the "mah" column, remove it from drop_columns.
# drop_columns = ['BIS NO.', 'BIS MODEL NO.', 'MAH', 'MADE IN', 'LOGO']  # Adjust as needed

# # If you need the "mah" column for substitution, ensure it is not dropped:
# if drop_columns and 'MAH' in drop_columns:
#     drop_columns.remove('MAH')
...
#     consolidated_start_row=6,
#     consolidated_start_col=10,
#     drop_columns=None  # or None if you want to keep all columns
# )
